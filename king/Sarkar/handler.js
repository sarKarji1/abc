...
const Handler = async (chatUpdate, sock, logger) => {
    try {
        ...
        if (m.key && m.key.remoteJid && m.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_SEEN) {
    try {
        console.log(`📢 Checking status from: ${m.key.remoteJid}`);

        // ✅ Ensure message key exists before marking seen
        if (m.key.id) {
            await sock.readMessages([m.key]);
            console.log("✅ Status marked as seen!");
        } else {
            console.log("⚠️ No valid message key found for status!");
        }
        ...
        await sock.sendMessage(m.key.remoteJid, { react: { text: randomReaction, key: m.key } });
        console.log(`✅ Reacted to status with: ${randomReaction}`);
    } catch (error) {
        console.error("❌ Error in status reaction:", error);
    }
}
